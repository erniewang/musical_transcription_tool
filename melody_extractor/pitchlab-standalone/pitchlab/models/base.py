...
def coerce_frequency_range(
    ...
    /,
    *,
    default: Optional[tuple[float, float] | FrequencyRange] = None,
    **kwargs: Any,
) -> Optional[FrequencyRange]:
    """
    Normalize any common range spelling into FrequencyRange.

    Accepted examples:
        coerce_frequency_range((100, 500))
        coerce_frequency_range(range=(100, 500))
        coerce_frequency_range(freq_range=(100, 500))
        coerce_frequency_range(fmin=100, fmax=500)
        coerce_frequency_range(min_frequency=100, max_frequency=500)
    """
    if isinstance(value, FrequencyRange):
        return value

    # Positional pair form: coerce_frequency_range((100, 500))
    if value is not None and not isinstance(value, Mapping):
        candidate_mapping = _as_mapping(value)
        if not candidate_mapping:
            low, high = _parse_pair(value)
            return FrequencyRange(low, high)
        data = candidate_mapping
    else:
    ...
    if pair is not None:
        low, high = _parse_pair(pair)
        return FrequencyRange(low, high)

    lows = [data[k] for k in LOW_KEYS if data.get(k) is not None]
    highs = [data[k] for k in HIGH_KEYS if data.get(k) is not None]
...
def remove_range_kwargs(kwargs: Mapping[str, Any]) -> dict[str, Any]:
    clean = dict(kwargs)
    for key in (*RANGE_PAIR_KEYS, *LOW_KEYS, *HIGH_KEYS):
        clean.pop(key, None)
    return clean
...
def hz_to_midi(frequency_hz: float | np.ndarray) -> float | np.ndarray:
    return 69.0 + 12.0 * np.log2(np.asarray(frequency_hz) / 440.0)


def midi_to_hz(midi_note: float | np.ndarray) -> float | np.ndarray:
    return 440.0 * np.power(2.0, (np.asarray(midi_note) - 69.0) / 12.0)
...
def normalize_audio_array(audio: Any) -> np.ndarray:
    ...
    # Accept channels-first or channels-last. Average the smaller dimension if plausible.
    if arr.ndim == 2:
        if arr.shape[0] <= 8:
            arr = arr.mean(axis=0)
        elif arr.shape[1] <= 8:
...
def filter_frequencies(
    ...
    *,
    ...
    arr = np.asarray(frequencies, dtype=float).copy()
    if freq_range is None:
        arr[~np.isfinite(arr)] = invalid_value
        return arr
    mask = (~np.isfinite(arr)) | (arr < freq_range.low_hz) | (arr > freq_range.high_hz)
...
def normalize_note_event(event: Any) -> Optional[dict[str, Any]]:
    if event is None:
        return None
...
def call_with_supported_kwargs(fn: Any, *args: Any, **kwargs: Any) -> Any:
    """Call function while dropping kwargs it cannot accept."""
    try:
...
class BasePitchModel:
    name = "base"
    aliases: tuple[str, ...] = ()
    default_range = FrequencyRange(50.0, 2000.0)
    supports_native_range = False
    output_type = "f0"

    def resolve_range(self, *, default: bool = True, **kwargs: Any) -> Optional[FrequencyRange]:
        default_value = self.default_range if default else None
        return coerce_frequency_range(kwargs, default=default_value)

    def predict(self, audio: Any, sample_rate: int, **kwargs: Any) -> pd.DataFrame:
        raise NotImplementedError

    def __call__(self, audio: Any, sample_rate: int, **kwargs: Any) -> pd.DataFrame:
        return self.predict(audio, sample_rate, **kwargs)

    @staticmethod
    def clean_audio(audio: Any) -> np.ndarray:
